...
`smart_match` нужен **не** чтобы написать «данные уже есть», а чтобы **дозаполнить/обновить** существующую запись из research (UPDATE по её `smart_id`). Всегда разбирайся, как заполнить Smart правильно, исходя из research, а не сообщай, что запись существует.
...
- **дубль/кросс-номера/«уже есть в Smart»** — это штатная работа (свести группу/дозаполнить/опубликовать), **НЕ** причина для review.
- **номер реально указывает на >1 разной детали**, и ни research, ни ты не можете развести — вот тогда review.

Утверждённый Smart (`is_unverified=false`/`is_draft=false`) до тебя не дойдёт — такие задачи закрываются ещё на research-стадии статусом `skipped_smart_approved`. Если всё же столкнулся с verified-записью, чей состав расходится с research, — не трогай, `mark_needs_review` с reason `smart_kit_verified_manual_review`.
