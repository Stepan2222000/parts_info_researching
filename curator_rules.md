...
`mark_needs_review` — крайняя мера, а не кнопка «отвалиться». Сначала пытайся **разобраться и опубликовать** (дозаполнить Smart из research, свести группу, починить формат). Помечай только узкий список настоящих тупиков:
...
- Smart kit зафиксирован: `is_unverified=false` и его состав расходится с research — reason `smart_kit_verified_manual_review`;
- номер реально указывает на >1 разной детали, и research не даёт развести.

Наличие записи в Smart, кросс-/устаревшие номера, дубль по группе — **НЕ** причины для `mark_needs_review`: это штатная работа (дозаполнить/свести/опубликовать).
