...
- Курсор следующего запроса = `latest_turn`. `since > latest_turn` → HTTP 400.
...
(по умолчанию **1200 сек**, см. `PARTS_RESEARCH_WAIT_TIMEOUT`). Формат entry —
как в `/research/submit` + `timed_out`. Клиентский HTTP-таймаут ставь **больше**
серверного потолка. `timed_out: true` → дозапрашивай `GET /research/{run_id}`
или поллинг `/turns`.
