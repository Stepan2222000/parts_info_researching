...
Реализуем curator на Vercel AI SDK v6 (раздел «Curator/write-agent» и подразделы). Используем `streamText` и `tools`. Поскольку фронт ещё не пишем, чат с куратором на этом этапе живёт через простой CLI-режим: пользователь запускает команду в терминале, она открывает сессию, шлёт сообщение и стримит ответ обратно в stdout. Этот CLI-режим — временный мост, чтобы протестировать куратора без UI. После этапа 4 он останется как удобный debug-инструмент.

Сессию хранят в БД полностью (раздел «Сессии куратора»): `curator_sessions`, `curator_messages`, плюс `agent_sql_log` для всех выполненных SQL. Никаких jsonl-файлов на диске для куратора — это чисто БД-история.

System-prompt куратора пересобирается перед каждым ответом и включает актуальное состояние очереди: сколько задач в `done` и ждут обработки, краткий список последних N задач (раздел «Когда запускается»). Это динамика, не статика.

Реализуем четыре tool`а куратора (раздел «Что у него есть»): `execute_sql`, `exa_search`, `save_to_smart`, `mark_needs_review`. `execute_sql` принимает сырой SQL без параметризации — агент пишет его сам. `exa_search` идёт через тот же кэширующий proxy, что и research-agent. `save_to_smart` — batch tool с per-row try/catch (раздел «save_to_smart batch tool»). `mark_needs_review` помечает задачу.

В system-prompt куратора прописываем все правила записи в Smart (раздел «Правила записи в Smart»): is_draft по умолчанию, is_unverified по умолчанию, маппинг полей, проверка бренда через `brand_aliases`, поведение по `is_draft = false` (не перезаписываем, только дополняем null — раздел «Конфликт с уже существующей Smart-записью»), правила для компонентов с/без артикулов (разделы «Компоненты с артикулами», «Компоненты без артикулов»), kit без состава (раздел «Kit без состава»). Эти правила должны быть прописаны словами в промпте, не закодированы в логику.

Реализуем таблицу `publications` и заполнение её куратором или его save_to_smart-tool`ом при каждой операции (раздел «Публикации»). Это даёт трассировку run → smart запись.

Проверяем, что параллельные tool calls Vercel AI SDK v6 действительно работают как нужно (раздел «Что у него есть»). До реализации прогоняем минимальный пример через терминал.

В конце этапа поднимаем production-деплой по шаблону из `DEPLOY_TEMPLATE.md` (раздел «Деплой»). Контейнеры: `parts_research_db` (Postgres 18 с `postgres_fdw`), `parts_research_app` (Next.js приложение — пока пустое, готовое принимать UI на этапе 4), `parts_research_worker` (worker-пул из этапа 2), `parts_research_exa_proxy` (наш кэширующий MCP-сервер). Все четыре висят в `db_default` сети рядом со `smart_test` и `brands_mapping`. Volume для `storage/` персистентный, `auth.json` Codex SDK тоже на персистентном volume с расчётом на write-back обновления токенов. GHA + Docker Build Cloud + ghcr.io + SSH deploy — по шаблону.

Перед деплоем прогоняем всё через терминал на сервере: worker запускается, Codex auth работает, MCP proxy слышен, FDW коннекты живы. После — даём worker`у обработать тестовую очередь и проверяем результаты.

**Что не делаем (откладывается на следующий этап).**

- Frontend — никакого UI, ни для очереди, ни для куратора. Куратор взаимодействует через CLI-мост. Это сознательно: реальный UI делаем сразу как production-quality в этапе 4, без промежуточных полу-готовых страниц.
...
**Готовность.** На production-сервере: запускаем worker, через CLI-мост открываем сессию куратора, добавляем в очередь 10–20 артикулов разной природы, ждём пока worker их обработает, пишем куратору в чат «обработай все», смотрим как он работает (через стрим в stdout и через `curator_messages` в БД). Проверяем `smart.parts`, `smart.part_brands`, `smart.part_components`, `smart.part_articles` — записи появились с правильными флагами. Проверяем `publications` — связь run → smart_id корректная. Проверяем сценарии: запчасть с одним брендом, запчасть с двумя брендами, набор с полным составом, набор с компонентами без артикулов, kit без состава (должен быть в `needs_human_review`, в Smart ничего), запчасть, которая уже была в Smart как draft, запчасть, которая уже была как `is_draft = false` (поля не перезаписаны).
...
Поднимаем frontend в `parts_research_app` контейнере. Используем Next.js (раздел «UI», «Стек»). Чат с куратором — через `useChat` из Vercel AI SDK v6, со стримом tool calls, чтобы видеть, что куратор делает в реальном времени.
...
Страница чата с куратором (раздел «Curator/write-agent → Когда запускается»). Сообщения и tool calls стримятся в реальном времени, видно какие SQL он выполняет, какие save_to_smart-операции делает, какие задачи помечает needs_human_review. История прошлых сессий доступна как лог. Можно открыть новый чат — это новая сессия в БД (раздел «Сессии куратора»).

CLI-мост из этапа 3 не убираем — он остаётся как полезный debug-инструмент.

Перед началом этапа делаем короткий поиск в интернете, чтобы убедиться, что используем актуальные API `useChat`, типизацию tool parts, способы стриминга кастомных data parts. Если что-то поменялось в v6 — учитываем.
